...
//    }
    ...
    stream.listen((Cart _cart) {
      if (!carts.contains(_cart)) {
        setState(() {
          coupon.value = _cart.food.applyCoupon(coupon.value);
          carts.add(_cart);
        });
      }
    }, onError: (a) {
  ...
  void doApplyCoupon(String code, {String message}) async {
    coupon.value = new Coupon.fromJSON({"code": code, "valid": null});
...
//          });
  ...
  Color getCouponIconColor() {
    if (coupon.value?.valid == true) {
      return Colors.green;
    } else if (coupon.value?.valid == false) {
